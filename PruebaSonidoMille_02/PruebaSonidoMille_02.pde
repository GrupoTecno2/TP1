...
void draw(){
  ...
  fRoja.cambiarAngulo(gPitch.filtradoNorm(),MIN_PITCH,MAX_PITCH,  40, 170);
  fRoja.dibujarTrazo();
  
  } if(paleta==2){
  fAzul.actualizar(gAmp.filtradoNorm(), gPitch.derivadaNorm());
  fAzul.cambiarAngulo(gPitch.filtradoNorm(),MIN_PITCH,MAX_PITCH,  40, 170);
  fAzul.dibujarTrazo();
  
  }if(paleta==3){
  fAmarilla.actualizar(gAmp.filtradoNorm(), gPitch.derivadaNorm());
  fAmarilla.cambiarAngulo(gPitch.filtradoNorm(),MIN_PITCH,MAX_PITCH,  40, 170);
